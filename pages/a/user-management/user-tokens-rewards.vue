  ...
  <div id="users-page" v-if="selected">
    <div class="container no-padding">
      <v-toolbar flat color="white">
        <v-toolbar-title>User My Rewards Token</v-toolbar-title>
          ...
          >
            ...
            <td>
              <div class="credit-wrapper container--flex container--flex-align-item-center">
                <span style="width: 30px;">{{ props.item.green_token }}</span>
                <input
                  type="number"
                  name="ken"
                  min="0"
                  v-model="props.item.green_token_add"
                  placeholder="0"
                ...
                <v-tooltip top>
                  <v-icon
                    ...
                    :class="{
                      'grey--text': props.item.green_token_option === 0 || isDisabled(props.item.id),
                      'black--text': props.item.green_token_option !== TOKEN_OPTIONS.ADD && !isDisabled(props.item.id),
                      'green--text': props.item.green_token_option === TOKEN_OPTIONS.ADD && !isDisabled(props.item.id),
                    }"
                ...
                <v-tooltip top>
                  <v-icon
                    ...
                    :class="{
                      'grey--text': props.item.green_token_option === 0 || isDisabled(props.item.id),
                      'black--text': props.item.green_token_option !== TOKEN_OPTIONS.MINUS && !isDisabled(props.item.id),
                      'red--text': props.item.green_token_option === TOKEN_OPTIONS.MINUS && !isDisabled(props.item.id),
                    }"
          ...
          <v-card-text style="padding: 25px;">
            ...
            <ul>
              <li
                ...
                :key="idx"
              >{{ i.name }} ({{ i.green_token_option === TOKEN_OPTIONS.ADD ? '+' : '-' }}{{ $pluralize('token', i.green_token_add) }})</li>
            </ul>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
