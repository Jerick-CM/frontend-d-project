<template>
  <div id="faq" class="container">
    ...
    <v-expansion-panel
        v-for="(categoryData, category) in faqCategories"
        v-bind:expand="true">
        <v-expansion-panel-content class="no-border"
          v-bind:expand-icon="'add'">
          ...
          <v-card>
            <v-card-text class="py-0">
              <v-expansion-panel
                ...
                <v-expansion-panel-content class="no-border"
                  v-bind:expand-icon="'add'"
                  v-for="(faq,idx) in categoryData"
                  v-bind:key="idx">
                  <div slot="header" class="m-md-vertical">
                    <h4 class="ml-2">{{ faq.title }}</h4>
                  </div>
                  <v-card>
                    <v-card-text class="ml-3 py-0 faq-content">
                      <div v-html="faq.content" class="ck-content"></div>
                    </v-card-text>
                  </v-card>
...
export default {
  async asyncData({ params }) {
    const { data } = await axios.get(
      process.env.API_URL + "/api/faq-n-contact-us"
    );
    const response = data.data;
    console.log(response);
    return {
      faqCategories: response.faq,
      faqPdf: response.faq_pdf,
    };
  },
  components: {
    ContactUs,
  ...
  methods: {
    ...
    downloadPDF() {
      window.open(this.faqPdf, "_blank");
    },
    clearPanel(panel) {
      this[panel] = [];
    },
